...
int main(int argc, char *argv[]) {
	...
	//packaging
	if(argc < 5) {
		printf("Usage : ./client  <fichier_a_envoyer> <fichier_recu> <adr_IP_dist> <port_dist> [<port_local>]");
		exit(1);
	}
	...
	file_recv = argv[2];
	adresse_dest = argv[3];
	port_distant = atoi(argv[4]);
	
	if(argc == 6) {
		port_local = atoi(argv[5]);
	...
	expediteur.sin_port = htons(port_distant);
	inet_aton(adresse_dest, &expediteur.sin_addr);


	//***************************************  DEBUT MEME CODE QUE SERVEUR ***************************************
	...
	dg_send->num_seq = '0'; //n° de sequence si 0 ou 1, 2 s'il n'y a pas de données
	dg_send->num_ack = '2'; //n° acquittement si 0 ou 1, 2 s'il n'y a pas d'acquittement
	int reSeq = 0, reAck = 0, eSeq = 0, eAck = 0, reSeqInter =0, eSeqEnd=-1, last_eAck;
	int ack_lastsend =0;
	int rcvLost = 0;
	int send_new = 1;
	...
	int k;
	for(k = eSeq; k < TAILLE_FENETRE; k++)
	{
		if(!send_finish)
		{
			dg_send->num_seq = eSeq+'0';
			if((k>=TAILLE_FENETRE || send_new|| firstsend))
			{
